...
bool MenuCameraBehaviour::Start()
{
	...
	// Spot light
	{
		...
		if (!GetScene()->CreateSimpleSpotLightEntity(&spotLight, "Spotlight Flashlight", { 30.0f, 30.0f, 30.0f }, 1.2f, 45.0f))
		{
			ErrMsg("Failed to create spotlight entity to flashlight!");
			return false;
		}
		spotLight->SetParent(GetEntity());
...
bool MenuCameraBehaviour::Update(Time &time, const Input &input)
{
	RaycastOut out;
...
bool MenuCameraBehaviour::Serialize(std::string *code) const
{
	// Standard code for Serialize
	*code += "MenuCameraBehaviour( )";

	return true;
}

bool MenuCameraBehaviour::Deserialize(const std::string &code)
{
	return true;
}
