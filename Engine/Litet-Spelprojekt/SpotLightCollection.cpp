...
bool SpotLightCollection::UpdateBuffers(ID3D11Device *device, ID3D11DeviceContext *context)
{
	...
	{
		SpotLightBehaviour *lightBehaviour = _lights.at(i).lightBehaviour;
	...
	if (_isDirty)
	{
		...
		{
			if (!_shadowCollection.Initialize(device, 1, 1, true, 1))
			{
		...
		{
			if (!_shadowCollection.Initialize(device, _texRes, _texRes, true, lightCount))
			{
				ErrMsg("Failed to initialize shadow collection!");
				return false;
			}
	...
	for (UINT i = 0; i < lightCount; i++)
	{
		...
		{
			ErrMsg(std::format("Failed to update spotlight #{} buffers!", i));
			return false;
		}
	}
	...
	{
		const SimpleSpotLightBehaviour *lightBehaviour = _simpleLights.at(i).lightBehaviour;
	...
	if (_isDirty)
	{
		...
		if (simpleLightCount <= 0) // Create empty simple light
		{
			...
			emptyLightBuffer.falloff = 1.0f;
			emptyLightBuffer.orthographic = -1;
...
bool SpotLightCollection::GetLightEnabled(UINT lightIndex) const
{
	...
	return _lights.at(lightIndex).isEnabled;
}
...
void SpotLightCollection::SetLightEnabled(UINT lightIndex, bool state)
{
	...
	_lights.at(lightIndex).isEnabled = state;
}
