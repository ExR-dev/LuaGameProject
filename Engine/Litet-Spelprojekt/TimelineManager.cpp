...
SequenceStatus TimelineManager::RunSequence(const std::string& sequenceName, Transform* transform, bool loop, bool lerpToStart)
{
	...
	{
		ErrMsg("Sequence not found: " + sequenceName);
	...
	_runningSequences.push_back(sequenceName);
	std::cout << "Running sequence " << sequenceName << std::endl;
...
bool TimelineManager::Update(Time& time)
{
...
bool TimelineManager::AddSequence(const std::string sequenceName, TimelineSequence sequence)
{
	...
	{
		ErrMsg("Sequence is invalid: " + sequence.GetError());
		return false;
	}
	...
	if (it != _sequences.end())
	{
		ErrMsg("Sequence already exists: " + sequenceName);
		return false;
	}

	_sequences.insert({ sequenceName, sequence });
	return true;
}
...
bool TimelineManager::RemoveSequence(const std::string sequenceName)
{
	...
	if (it->second.GetStatus() == SequenceStatus::RUNNING)
	{
		ErrMsg("Cannot remove running sequence: " + sequenceName);
		return false;
	}
...
bool TimelineManager::Serialize(std::string *code) const
{
	...
	for (auto& sequence : _sequences)
	{
		...
		*code += ")";
		...
		{
			ErrMsg("Failed to serialize sequence: " + sequence.first);
			return false;
		}
	...
	std::ofstream file("Content/Saves/Sequences.txt", std::ios::out);
	if (!file)
	{
...
bool TimelineManager::Deserialize()
{
	...
	// Read file
	std::ifstream file("Content/Saves/Sequences.txt", std::ios::in);
	if (!file)
	{
		ErrMsg("Could not open sequences file.");
		return false;
	...
	while (std::getline(file, line))
	{
		...
		{
			ErrMsg("Failed to deserialize sequence: " + sequenceName);
			return false;
		}
				...
				if (ImGui::Button("Remove waypoint"))
				{
					if (!waypoints.empty())
					{
						if (newSequence.RemoveLastWaypoint())
							std::cout << "Waypoint removed." << std::endl;
					}
				}
			...
			// Finish sequence button
			{
				...
				if (ImGui::Button("Finish"))
				{
					if (!newSequence.Validate())
					{
						...
						ErrMsg("Sequence is invalid: " + error);
...
TimelineSequence TimelineManager::GetSequence(std::string sequenceName) const
{
	auto it = _sequences.find(sequenceName);
	if (it == _sequences.end())
	{
