...
std::string ToString(const MonsterStatus& state)
{
	switch (state)
	{
...
bool MonsterBehaviour::AnyStateUpdate(Time &time)
{	
	...
	// Alert: Growth/Decay
	if (_playerInSight)
		GrowAlert(time.deltaTime);
	else
		DecayAlert(time.deltaTime);

	// Capture player if the criteria are met
	if (_playerDistance < _playerCaptureDistance)
	{
		if (!_playerMover->IsHiding())
			SetState(MonsterStatus::CAPTURE);
...
void MonsterBehaviour::MoveToPoint(Time &time, const XMFLOAT3A &point)
{
	...
	XMFLOAT4A moveDir;
	Store(moveDir, XMVectorScale(moveDirV, _moveSpeed * _movementFactor * time.deltaTime));
...
bool MonsterBehaviour::HasLOS(XMFLOAT3A &point)
{
	...
	float dist;
	XMStoreFloat(&dist, XMVector3Length(dirVec));
...
bool MonsterBehaviour::SetState(const MonsterStatus& newState)
{
	if (_state == newState)
		return true;
	...
	std::cout << "State is now " << ToString(_state) << std::endl;

	return true;
}

bool MonsterBehaviour::DecayAlert(const float& deltaTime)
{
	if (_alertLevel > 0)
...
bool MonsterBehaviour::GrowAlert(const float &deltaTime)
{
...
void MonsterBehaviour::GetRandomizedNodePos(XMFLOAT3A &pos, bool mineOnly)
{
	...
	if (nodes.size() > 0)
	{
		GraphNodeBehaviour *node = nodes.at(rand() % nodes.size());
...
void MonsterBehaviour::PathFind(Time &time)
{
	...
	if (!_path.empty())
	{
		XMVECTOR posToPointVec = XMVectorSubtract(Load(_path.at(0)), Load(GetTransform()->GetPosition(World)));
		XMVECTOR lengthVec = XMVector3Length(posToPointVec);
		...
		if (!_path.empty())
		{
			...
			if (nodeUpdate)
			{
				...
				if (_graphManager->isPoint(_path.at(0)) && !_graphManager->isMinePoint(_path.at(0))) _nextNodeIsCave = true;
			...
			MoveToPoint(time, To3(_path.at(0)));

			// Display first node
			//XMFLOAT3 p1 = _path.at(0);
			//XMFLOAT3 p2 = p1;
			//p2.y += 10.0f;
...
bool MonsterBehaviour::Start()
{
	...
	_spawnPositions.push_back({-122.0f, -15.0f, 5.0f });
	_spawnPositions.push_back({ -122.0f, -3.0f, 166.0f });
	_spawnPositions.push_back({ 173.0f, 11.0f, 92.0f });
	_spawnPositions.push_back({ 165.0f, 3.0f, -88.0f });
	_spawnPositions.push_back({ -94.0f, -11.0f, -204.0f });
	_spawnPositions.push_back({ -284.0f, -1.0f, -112.0f });
	...
	Content *sceneContent = GetScene()->GetContent();
	if (!sceneContent)
	{
	...
	// Set mesh
	UINT meshID = sceneContent->GetMeshID("Mesh_DecimatedMan");
	Material mat = Material::MakeMat(
		sceneContent->GetTextureID("Tex_Black"),
		CONTENT_NULL,
		sceneContent->GetTextureMapID("TexMap_Black_Specular"),
		sceneContent->GetTextureMapID("TexMap_Black_Glossiness"),
		sceneContent->GetTextureID("Tex_Black")
	);
	DirectX::BoundingOrientedBox bounds = sceneContent->GetMesh(meshID)->GetBoundingOrientedBox();
...
bool MonsterBehaviour::Update(Time& time, const Input& input)
{
	if (!_playerEntity)
	{
		_playerEntity = GetScene()->GetSceneHolder()->GetEntityByName("Player Entity");
...
bool MonsterBehaviour::FixedUpdate(const float& deltaTime, const Input& input)
{
	return true;
}
...
bool MonsterBehaviour::RenderUI()
{
	...
	if (!_path.empty())
	{
		std::string nextPoint = "Next Point: " + std::to_string(_path.at(0).x) + ", " + std::to_string(_path.at(0).y) + ", " + std::to_string(_path.at(0).z);
...
bool MonsterBehaviour::Alert(const float amount, std::optional<DirectX::XMFLOAT3A> alertOrigin, 
	std::optional<const float> distance, std::optional<bool> scaleWithDist, std::optional<bool> requireLOS)
{
	...
	if (alertOrigin)
	{
		...
		DirectX::XMFLOAT3A pos = t->GetPosition(World);
		DirectX::XMFLOAT3A origin = alertOrigin.value();

		float rad = distance ? distance.value() : _hearingRange;
		float distSqr = 
...
bool MonsterBehaviour::soundAlert(const float amount, std::optional<DirectX::XMFLOAT3A> soundOrigin, 
	std::optional<const float> reach, std::optional<bool> scaleWithDist)
{
	...
	float dist;
	XMStoreFloat(&dist, XMVector3Length(Load(soundOrigin.value()) - Load(GetTransform()->GetPosition())));
...
bool MonsterBehaviour::Serialize(std::string *code) const
{
	*code += _name + "(";

	*code += std::to_string(_alertLevel);

	*code += " )";
	return true;
}
bool MonsterBehaviour::Deserialize(const std::string &code)
{
