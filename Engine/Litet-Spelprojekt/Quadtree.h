...
private:
	...
	struct Node
	{
		...
		void Split(const UINT depth)
		{
			...
			for (int i = 0; i < data.size(); i++)
			{
				if (data[i] != nullptr)
				{
					...
					{
						DirectX::XMFLOAT3A pos = data[i]->GetTransform()->GetPosition();
		...
		void BoxCull(const DirectX::BoundingBox &box, std::vector<Entity *> &containingItems, const UINT depth = 0) const
		{
			switch (box.Contains(bounds))
			{
			case DirectX::DISJOINT:
