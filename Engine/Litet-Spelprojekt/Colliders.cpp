...
bool Collisions::Collider::SetDebugEnt(Scene* scene, const Content* content)
{
	...
	const UINT
		...
		textureID = content->GetTextureID("Tex_Green"),
		normalID = CONTENT_NULL,
		specularID = CONTENT_NULL,
		reflectiveID = CONTENT_NULL,
		ambientID = content->GetTextureID("Tex_Green"),
	...
	*/
	...
	Material mat;
	mat.textureID = content->GetTextureID("Tex_Green");
	mat.ambientID = content->GetTextureID("Tex_Green");
...
DirectX::XMFLOAT3 Collisions::Collider::GetMin() const
{
	return _min;
}

DirectX::XMFLOAT3 Collisions::Collider::GetMax() const
{
	return _max;
}
...
bool Collisions::Collider::Serialize(std::string *code) const
{
	return true;
}

bool Collisions::Collider::Deserialize(const std::string &code)
{
	return true;
}
...
Collisions::Ray::Ray(const DirectX::XMFLOAT3 &o, const DirectX::XMFLOAT3 &d, float l, ColliderTags tag):
	Collider(RAY_COLLIDER, "Mesh_WireframeCube", tag), origin(o), dir(d), length(l)
{
}
...
bool Collisions::Ray::RenderUI()
{
	...
	_isDirty |= ImGui::DragScalarN("Origon", ImGuiDataType_Float, o.m128_f32, 3, speed, minValues, maxValues, "%.3f");
	_isDirty |= ImGui::DragScalarN("Direction", ImGuiDataType_Float, d.m128_f32, 3, speed, minValues, maxValues, "%.3f");
	_isDirty |= ImGui::DragScalar("Lengt", ImGuiDataType_Float, &length, speed, minValues, maxValues, "%.3f");
...
bool Collisions::Ray::RenderDebug(Scene *scene, const RenderQueuer &queuer, const RendererInfo &renderInfo)
{
	...
	DebugDrawer *debugDraw = scene->GetDebugDrawer();
	XMFLOAT3 max;
	XMStoreFloat3(&max, XMVectorAdd(Load(origin), XMVectorScale(Load(dir), length)));
...
void Collisions::Ray::Transform(DirectX::XMFLOAT4X4A M, Collider *transformed)
{
	if (!transformed)
		return;
	...
	XMStoreFloat3(&transformedRay->origin, XMVector3Transform(XMLoadFloat3(&origin), worldMatrix));

	XMVECTOR axisVec = XMLoadFloat3(&dir);
	axisVec = XMVector3Normalize(XMVector3TransformNormal(axisVec, worldMatrix));
	XMStoreFloat3(&transformedRay->dir, axisVec);
	...
	XMStoreFloat3(&transformedRay->_min, XMVectorMin(startPoint, endPoint));
	XMStoreFloat3(&transformedRay->_max, XMVectorMax(startPoint, endPoint));
...
#ifdef DEBUG_BUILD
bool Collisions::Ray::TransformDebug(ID3D11DeviceContext* context, Time &time, const Input &input)
{
	return true;
}
#endif
...
Collisions::Sphere::Sphere(const DirectX::XMFLOAT3& c, float r, ColliderTags tag):
	Collider(SPHERE_COLLIDER, "Mesh_WireframeSphere", tag), center(c), radius(r)
{
}

Collisions::Sphere::Sphere(const DirectX::BoundingSphere& sphere, ColliderTags tag):
	Collider(SPHERE_COLLIDER, "Mesh_WireframeSphere", tag), center(sphere.Center), radius(sphere.Radius)
{
}
...
void Collisions::Sphere::Transform(DirectX::XMFLOAT4X4A M, Collider *transformed)
{
	if (!transformed)
		return;
	...
	// Translation
	XMStoreFloat3(&transformedSphere->center, XMVector3Transform(XMLoadFloat3(&center), worldMatrix));

	// Scaling
	XMFLOAT3 scale;
    scale.x = XMVectorGetX(XMVector3Length(worldMatrix.r[0])) * radius;
    scale.y = XMVectorGetX(XMVector3Length(worldMatrix.r[1])) * radius;
    scale.z = XMVectorGetX(XMVector3Length(worldMatrix.r[2])) * radius;
...
#ifdef DEBUG_BUILD
bool Collisions::Sphere::TransformDebug(ID3D11DeviceContext* context, Time &time, const Input &input)
{
	if (!debug || !_debugEnt)
		return true;

	_debugEnt->GetTransform()->SetPosition(XMFLOAT3A(center.x,
													 center.y, 
													 center.z));

	_debugEnt->GetTransform()->SetScale(XMFLOAT4A(radius, radius, radius, 0));

	if (!_debugEnt->GetTransform()->UpdateConstantBuffer(context))
	{
...
Collisions::Capsule::Capsule(const DirectX::XMFLOAT3 &c, const DirectX::XMFLOAT3 &u, float r, float h, ColliderTags tag):
	Collider(CAPSULE_COLLIDER, "Mesh_WireframeCapsule", tag), center(c), upDir(u), radius(r), height(h)
{
	if (radius * 2 > height)
		radius = height / 2;
}
...
void Collisions::Capsule::Transform(DirectX::XMFLOAT4X4A M, Collider *transformed)
{
	if (!transformed)
		return;
	...
	// Translation
	XMStoreFloat3(&transformedCapsule->center, XMVector3Transform(XMLoadFloat3(&center), worldMatrix));
	...
	XMStoreFloat3(&transformedCapsule->upDir, XMVector3Normalize(axisVec));


	// Scaling
	XMFLOAT3 scale;
    scale.x = XMVectorGetX(XMVector3Length(worldMatrix.r[0])) * radius;
    scale.y = XMVectorGetX(XMVector3Length(worldMatrix.r[1])) * height;
    scale.z = XMVectorGetX(XMVector3Length(worldMatrix.r[2])) * radius;
	...
	XMStoreFloat3(&transformedCapsule->_min, XMVectorMin(XMVectorMin(XMVectorMin(p1, p2), p3), p4));
	XMStoreFloat3(&transformedCapsule->_max, XMVectorMax(XMVectorMax(XMVectorMax(p1, p2), p3), p4));

	_isDirty = false;
}

#ifdef DEBUG_BUILD
bool Collisions::Capsule::TransformDebug(ID3D11DeviceContext *context, Time &time, const Input &input)
{
	if (!_debugEnt || !debug)
		return true;

	// Translation
	_debugEnt->GetTransform()->SetPosition(XMFLOAT4A(center.x,
													 center.y, 
													 center.z, 
													 0));
	...
	{
		_debugEnt->GetTransform()->SetRotation({ 0, 0, 0, 1 });
	}
	else if (dot <= -1.0f)
	{
		...
		_debugEnt->GetTransform()->SetRotation(quaternion);
	}
	else
	{
	...
	// Scaling
	_debugEnt->GetTransform()->SetScale(XMFLOAT3A(radius, height/2, radius));
...
Collisions::OBB::OBB(const DirectX::XMFLOAT3 &c, const DirectX::XMFLOAT3 &hl, const DirectX::XMFLOAT3 a[3], ColliderTags tag):
    Collider(OBB_COLLIDER, "Mesh_WireframeCube", tag),	center(c), halfLength(hl)
{
	for (int i = 0; i < 3; i++)
		axes[i] = a[i];
}
...
Collisions::OBB::OBB(const BoundingOrientedBox &obb, ColliderTags tag):
	Collider(OBB_COLLIDER, "Mesh_WireframeCube", tag), center(obb.Center), halfLength(obb.Extents)
{
	XMVECTOR orientation = XMLoadFloat4(&obb.Orientation);

	XMStoreFloat3(&axes[0], XMVector3Rotate({1, 0, 0}, orientation));
	XMStoreFloat3(&axes[1], XMVector3Rotate({ 0, 1, 0 }, orientation));
	XMStoreFloat3(&axes[2], XMVector3Rotate({0, 0, 1}, orientation));
}

Collisions::OBB::OBB(const AABB &aabb, ColliderTags tag):
	Collider(OBB_COLLIDER, "Mesh_WireframeCube", tag), center(aabb.center), halfLength(aabb.halfLength)
{
	axes[0] = { 1, 0, 0 };
	axes[1] = { 0, 1, 0 };
	axes[2] = { 0, 0, 1 };
...
bool Collisions::OBB::RenderUI()
{
	...
	XMStoreFloat3(&center, c);
	XMStoreFloat3(&halfLength, hl);
	for (int i = 0; i < 3; i++)
	{
		a[i] = XMVector3Normalize(a[i]);
		XMStoreFloat3(&axes[i], a[i]);
	}
...
void Collisions::OBB::Transform(DirectX::XMFLOAT4X4A M, Collider *transformed)
{
	if (!transformed)
		return;
	...
	// Transform center position
	XMStoreFloat3(&transformedOBB->center, XMVector3Transform(XMLoadFloat3(&center), worldMatrix));
    ...
    for (int i = 0; i < 3; i++)
    {
        ...
        axisVec = XMVector3TransformNormal(axisVec, worldMatrix);
        XMStoreFloat3(&transformedOBB->axes[i], XMVector3Normalize(axisVec));
    }
    ...
    XMStoreFloat3(&transformedOBB->halfLength, XMVectorAbs(XMVectorMultiply(XMLoadFloat3(&halfLength), XMLoadFloat3(&scale))));
	...
	XMStoreFloat3(&transformedOBB->_min, minPoint);
	XMStoreFloat3(&transformedOBB->_max, maxPoint);

	_isDirty = false;
}

#ifdef DEBUG_BUILD
bool Collisions::OBB::TransformDebug(ID3D11DeviceContext *context, Time &time, const Input &input)
{
	if (!_debugEnt || !debug)
		return true;
	...
	XMFLOAT4A rot;
	XMStoreFloat4A(&rot, XMQuaternionRotationMatrix(XMMatrixSet(axes[0].x, axes[0].y, axes[0].z, 0,
																axes[1].x, axes[1].y, axes[1].z, 0,
																axes[2].x, axes[2].y, axes[2].z, 0,
																0, 0, 0, 1)));

	_debugEnt->GetTransform()->SetRotation(rot);

	_debugEnt->GetTransform()->SetScale(XMFLOAT4A(halfLength.x, 
												  halfLength.y, 
												  halfLength.z, 
												  0));
...
Collisions::AABB::AABB(const DirectX::XMFLOAT3& c, const DirectX::XMFLOAT3& hl, ColliderTags tag):
	Collider(AABB_COLLIDER, "Mesh_WireframeCube", tag), center(c), halfLength(hl)
{
}

Collisions::AABB::AABB(const DirectX::BoundingBox& aabb, ColliderTags tag):
	Collider(AABB_COLLIDER, "Mesh_WireframeCube", tag), center(aabb.Center), halfLength(aabb.Extents)
{
}

Collisions::AABB::AABB(const DirectX::BoundingOrientedBox &obb, ColliderTags tag):
	Collider(AABB_COLLIDER, "Mesh_WireframeCube", tag), center(obb.Center), halfLength(obb.Extents)
{
}
...
void Collisions::AABB::Transform(DirectX::XMFLOAT4X4A M, Collider *transformed)
{
	if (!transformed)
		return;
	...
	// Translation
	XMStoreFloat3(&transformedAABB->center, XMVector3Transform(XMLoadFloat3(&center), worldMatrix));
	...
	XMStoreFloat3(&transformedAABB->halfLength, maxAxis);
...
#ifdef DEBUG_BUILD
bool Collisions::AABB::TransformDebug(ID3D11DeviceContext* context, Time &time, const Input &input)
{
	if (!_debugEnt || !debug)
		return true;
...
Collisions::Terrain::Terrain(const Terrain &other):
	Collider(TERRAIN_COLLIDER, "Mesh_WireframeCube", (ColliderTags)other._tagFlag)
{
...
Collisions::Terrain::Terrain(const DirectX::XMFLOAT3 &c, const DirectX::XMFLOAT3 &hl, const HeightMap *heightMap, ColliderTags tag, bool invertHeight, bool isWallCollider):
	Collider(TERRAIN_COLLIDER, "Mesh_WireframeCube", tag)
{
...
Collisions::Terrain::Terrain(const DirectX::BoundingBox &aabb, const HeightMap *heightMap, ColliderTags tag, bool invertHeight, bool isWallCollider):
...
void Collisions::Terrain::Init(const DirectX::XMFLOAT3 &c, const DirectX::XMFLOAT3 &hl, const std::vector<float> &heightMap, UINT width, UINT height, ColliderTags tag)
{
	...
	if (_isWallCollider)
		walls.resize(tileSize.x * tileSize.y);
	else
		heightValues.resize(tileSize.x * tileSize.y);
...
bool Collisions::Terrain::GetHeight(UINT x, UINT y, float &height) const
{
	...
	if (x < static_cast<UINT>(tileSize.x) && y < static_cast<UINT>(tileSize.y))
	{
		...
		height = (center.y - scalar * halfLength.y) + scalar * heightValues[(tileSize.y - 1 - y) * tileSize.x + x] * halfLength.y * 2;
		return true;
	}
	return false;
}

bool Collisions::Terrain::GetHeight(const DirectX::XMFLOAT2 pos, float &height) const
{
	if (_isWallCollider)
		return false;
...
bool Collisions::Terrain::GetHeight(const DirectX::XMFLOAT2 pos, float &height, DirectX::XMFLOAT3 &normal) const
{
	if (_isWallCollider)
		return false;
...
bool Collisions::Terrain::GetNormal(UINT x, UINT y, DirectX::XMFLOAT3 &normal) const
{
	if (_isWallCollider)
		return false;
	...
	h00 = heightValues[(tileSize.y - 1 - baseY) * tileSize.x + baseX-1];
	h10 = heightValues[(tileSize.y - 1 - baseY) * tileSize.x + baseX+1];
	h01 = heightValues[(tileSize.y - 1 - (baseY-1)) * tileSize.x + baseX];
	h11 = heightValues[(tileSize.y - 1 - (baseY+1)) * tileSize.x + baseX];
...
bool Collisions::Terrain::GetNormal(const DirectX::XMFLOAT2 pos, DirectX::XMFLOAT3 &normal) const
{
	if (_isWallCollider)
		return false;
...
bool Collisions::Terrain::IsWall(const DirectX::XMFLOAT2 pos) const
{
	if (!_isWallCollider)
		return false;
...
bool Collisions::Terrain::IsWall(UINT x, UINT y) const
{
	...
	{
		int scalar = _invertHeight ? -1 : 1;
		return walls[(tileSize.y - 1 - y) * tileSize.x + x];
	}
	return true; // Outside of terrain
}

DirectX::XMFLOAT2 Collisions::Terrain::GetTileWorldSize() const
{
...
bool Collisions::Terrain::RenderDebug(Scene *scene, const RenderQueuer &queuer, const RendererInfo &renderInfo)
{
	...
	for (int i = 0; i < tileSize.x; i++)
	{
		...
		{ 
			...
			debugDraw->DrawLineThreadSafe(min, max, 0.01f, color);
		}
	}
...
bool Collisions::Terrain::Serialize(std::string *code) const
{
...
void Collisions::Terrain::Transform(DirectX::XMFLOAT4X4A M, Collider *transformed)
{
	if (!transformed)
		return;
	...
	// Translation
	XMStoreFloat3(&transformedTerrain->center, XMVector3Transform(XMLoadFloat3(&center), worldMatrix));
	...
	XMStoreFloat3(&transformedTerrain->halfLength, maxAxis);
...
#ifdef DEBUG_BUILD
bool Collisions::Terrain::TransformDebug(ID3D11DeviceContext *context, Time &time, const Input &input)
{
	return true;
}
#endif
