...
{
	float factor = 0;
	DirectX::XMFLOAT3A startPos;
	DirectX::XMVECTOR startQuat;
	DirectX::XMFLOAT3A startScale;

	DirectX::XMFLOAT3A endPos = end->GetPosition();
	DirectX::XMVECTOR endQuat = DirectX::XMLoadFloat4A(&end->GetRotation());
	DirectX::XMFLOAT3A endScale = end->GetScale();
	...
	if (start == nullptr)
	{
		...
		startQuat = DirectX::XMLoadFloat4A(&_startRotation);
	...
	else
	{
		...
		startQuat = DirectX::XMLoadFloat4A(&start->GetRotation());
	...
	DirectX::XMFLOAT3A position = DirectX::XMFLOAT3A(
	...
	DirectX::XMStoreFloat4A(&rotation, DirectX::XMQuaternionSlerp(startQuat, endQuat, factor));

	// Interpolate the scale.
	DirectX::XMFLOAT3A scale = DirectX::XMFLOAT3A(
...
SequenceStatus TimelineSequence::Update(Time& time)
{
...
bool TimelineSequence::Serialize(std::string* code) const
{
	...
	for (auto& waypoint : _waypoints)
	{
		...
		*code += "<";
		...
		{
			ErrMsg("Failed to serialize waypoint.");
			return false;
		}
		*code += ">";
	}
...
bool TimelineSequence::Deserialize(const std::string& code)
{
	...
	while ((start = code.find('<', end)) != std::string::npos &&
		...
		waypoints.push_back(code.substr(start + 1, end - start - 1));
	}
	...
	for (size_t i = 0; i < waypoints.size(); i++)
	{
		...
		{
			ErrMsg("Failed to deserialize waypoint " + std::to_string(i));
			return false;
		}
...
TimelineWaypoint TimelineSequence::GetWaypoint(int index)
{
	if (index < 0 || index >= _waypoints.size())
	{
		ErrMsg("Invalid waypoint index: " + std::to_string(index));
		return TimelineWaypoint();
	}
