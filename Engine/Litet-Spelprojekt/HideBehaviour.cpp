...
bool HideBehaviour::Start()
{
	...
	SoundBehaviour *sound = new SoundBehaviour("LockerDoorSound", DirectX::SOUND_EFFECT_INSTANCE_FLAGS::SoundEffectInstance_Default);
...
bool HideBehaviour::Serialize(std::string *code) const
{
	*code += _name + "( )";
...
bool HideBehaviour::Deserialize(const std::string &code)
{
	return true;
}
...
void HideBehaviour::Hide()
{
	...
	if (pmb->IsHiding()) // Player is already hiding -> Exit hiding spot
	{
		...
		DirectX::XMFLOAT3A objectPos = _objectTransform->GetPosition(World);
		DirectX::XMFLOAT3A objectFor = _objectTransform->GetForward(World);

		// Move player forward 1 meter in front of hiding spot
		DirectX::XMFLOAT3A newPlayerPos = {};
		newPlayerPos.x = objectPos.x + objectFor.x;
		newPlayerPos.y = objectPos.y + objectFor.y;
		newPlayerPos.z = objectPos.z + objectFor.z;
		_playerEntity->GetTransform()->SetPosition(DirectX::XMFLOAT3A(newPlayerPos.x, newPlayerPos.y, newPlayerPos.z), World); // Change y to ground height
	...
	else if (!pmb->IsHiding()) // Player is not hiding -> Enter hiding spot
	{
		...
		DirectX::XMFLOAT3A objectPos = _objectTransform->GetPosition(World);
		objectPos.y += 0.2; // does nothing apparently
