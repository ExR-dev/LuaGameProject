...
const DirectX::XMFLOAT3A& TimelineWaypoint::GetPosition() const
{
	return _worldPosition;
}

const DirectX::XMFLOAT4A& TimelineWaypoint::GetRotation() const
{
	return _worldRotation;
}

const DirectX::XMFLOAT3A& TimelineWaypoint::GetScale() const
{
	return _worldScale;
}
...
bool TimelineWaypoint::Deserialize(const std::string& code)
{
	...
	{
		components.push_back(item.substr(1, item.size() - 2)); // Remove parentheses
	}
	...
	while (std::getline(ss, item, ','))
	{
		posComponents.push_back(std::stof(item));
	}
	_worldPosition = DirectX::XMFLOAT3A(posComponents[0], posComponents[1], posComponents[2]);
	...
	while (std::getline(ss, item, ','))
	{
		rotComponents.push_back(std::stof(item));
	}
	_worldRotation = DirectX::XMFLOAT4A(rotComponents[0], rotComponents[1], rotComponents[2], rotComponents[3]);
	...
	while (std::getline(ss, item, ','))
	{
		scaleComponents.push_back(std::stof(item));
	}
	_worldScale = DirectX::XMFLOAT3A(scaleComponents[0], scaleComponents[1], scaleComponents[2]);
