...
bool InventoryBehaviour::Start()
{
	...
	// Held breadcrumb
	{
		...
		Material mat{};
		mat.textureID = content->GetTextureID("Tex_Breadcrumb_Red");
		mat.normalID = content->GetTextureMapID("TexMap_Breadcrumb_Normal");
		mat.specularID = content->GetTextureMapID("TexMap_Breadcrumb_Red_Specular");
		mat.glossinessID = content->GetTextureMapID("TexMap_Breadcrumb_Glossiness");
		mat.lightID = content->GetTextureID("Tex_Breadcrumb_Red_Light");
...
bool InventoryBehaviour::Update(Time &time, const Input &input)
{
...
bool InventoryBehaviour::Serialize(std::string *code) const
{
	*code += _name + "(" + std::to_string(_breadcrumbCount) + " )";

	return true;
}
bool InventoryBehaviour::Deserialize(const std::string &code)
{
	...
	_breadcrumbCount = attributes.at(0);
