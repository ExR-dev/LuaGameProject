...
bool PlayerCutsceneBehaviour::Start()
{
	...
	_dragSound = new SoundBehaviour("DragBody", (DirectX::SOUND_EFFECT_INSTANCE_FLAGS)(0x3), false, 50.0f, 0.25f);
    ...
    _caveMusic = new SoundBehaviour("The Cave", DirectX::SoundEffectInstance_ReverbUseFilters, false, 50.0f, 0.25f);
    ...
    _faintSound = new SoundBehaviour("BodyCollapse", DirectX::SoundEffectInstance_ReverbUseFilters, false, 50.0f, 0.25f);
...
bool PlayerCutsceneBehaviour::Update(Time &time, const Input &input)
{
    ...
    case DONE:
        if (_currentTimer.IsDone())
        {
            ...
            if (!GetScene()->GetGame()->SetScene(GAME_SCENE))
            {
