...
PixelShaderOutput main(PixelShaderInput input)
{
    ...
    const float glossiness = (sampleGlossiness > 0)
		//? 1.0 / (1.0 - GlossinessMap.Sample(Sampler, input.tex_coord).x)
		? specularExponent * pow(GlossinessMap.Sample(Sampler, input.tex_coord).x, 2.0)
		: specularExponent;
    ...
    float3 totalLight = occlusion * totalSpecularLight;
