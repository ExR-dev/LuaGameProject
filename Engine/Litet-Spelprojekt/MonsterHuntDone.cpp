...
bool MonsterHuntDone::OnUpdate(Time &time)
{
	...
	_mb->_alertLevel += distanceFactor * _mb->_playerNoiseLevel * time.deltaTime;

	// Hiding with flashlight on
	if (_mb->_playerMover->IsHiding() && _mb->_playerFlashlight->IsOn())
		_mb->_alertLevel += 25.0f * time.deltaTime;

	// Decrement sound
	_mb->_currentSoundVolume -=  2 * _mb->_soundVolumes[_mb->_currentSound] * _mb->_soundVolumeIncreaseRate * time.deltaTime;
