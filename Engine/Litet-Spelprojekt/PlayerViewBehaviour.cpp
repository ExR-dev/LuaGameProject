...
bool PlayerViewBehaviour::Update(Time &time, const Input &input)
{
...
bool PlayerViewBehaviour::FixedUpdate(const float &deltaTime, const Input &input)
{
	if (!_movement)
		return true;
...
bool PlayerViewBehaviour::Serialize(std::string *code) const
{
	*code += _name + "( )";
	return true;
}

bool PlayerViewBehaviour::Deserialize(const std::string &code)
{
	SceneHolder *sceneHolder = GetScene()->GetSceneHolder();
	Entity *playerEnt = sceneHolder->GetEntityByName("Player Entity");
	if (playerEnt)
